    ...
    def archive_html_with_assets(self, html_path, html_content, pub_slug):
        ...
        for img_tag in soup.find_all('img'):
            ...
            try:
                ...
                # Check if content is actually a zip file (magic bytes)
                is_zip = content[:4] == b'PK\x03\x04'
                if is_zip:
                    print(f"       ⚠️  WARNING: Downloaded content is a ZIP file!")
                    print(f"       This URL returned a zip instead of an image: {absolute_url[:80]}")
                ...
                if '.' in original_filename:
                    ...
                    if is_zip:
                        ext = '.zip'
                        print(f"       Using .zip extension due to file signature")
                    else:
