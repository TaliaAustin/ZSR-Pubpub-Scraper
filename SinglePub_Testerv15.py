    ...
    def archive_html_with_assets(self, html_path, html_content, pub_slug):
        ...
        for img_tag in soup.find_all('img'):
            ...
            try:
                ...
                # DEBUG: Check what we're actually downloading
                ...
                file_signature = content[:20].hex() if len(content) >= 20 else ''
                print(f"       🔍 File signature (first 20 bytes): {file_signature}")
                
                # Determine extension from file signature (most reliable)
                ext = None
                if content[:4] == b'\xff\xd8\xff\xe0' or content[:4] == b'\xff\xd8\xff\xe1':
                    ...
                    ext = '.gif'
                    print(f"       ✅ Detected: GIF image")
                elif content[:4] == b'RIFF' and content[8:12] == b'WEBP':
                    ext = '.webp'
                    print(f"       ✅ Detected: WebP image")
                elif content[:2] == b'BM':
                    ext = '.bmp'
                    print(f"       ✅ Detected: BMP image")
                elif content[:4] == b'<svg' or b'xmlns="http://www.w3.org/2000/svg"' in content[:200]:
                    ext = '.svg'
                    print(f"       ✅ Detected: SVG image")
                elif content[:4] == b'PK\x03\x04':
                    ext = '.zip'
                    print(f"       ⚠️  WARNING: ZIP file, not an image!")
                ...
                if not ext:
                    ...
                    if '.' in original_filename:
                        ...
                        print(f"       📝 Using extension from URL: {ext}")
                
                # Fallback to content-type
                if not ext:
                    ext = mimetypes.guess_extension(content_type)
                    if ext:
                        print(f"       📝 Using extension from Content-Type: {ext}")
                
                # Last resort
                if not ext:
                    ext = '.jpg'  # Default to .jpg instead of .bin
                    print(f"       ⚠️  Could not detect type, defaulting to .jpg")
